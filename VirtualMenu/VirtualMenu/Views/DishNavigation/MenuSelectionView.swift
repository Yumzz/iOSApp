...
struct MenuSelectionView: View {
    ...
    var body: some View {
        VStack{
            VStack(spacing: 60){
                NavigationLink(destination: ListDishesView(dishes: self.restChosen.dishes!, rest: self.restChosen)){
                    MenuButton()
                }
                .buttonStyle(PlainButtonStyle())
                
                NavigationLink(destination: ScanView(dishes: self.restChosen.dishes!, rest: self.restChosen)){
                    ScannerButton()
                }
                .buttonStyle(PlainButtonStyle())
            }
